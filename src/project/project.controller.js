...
const addUserToProject = async (req, res) => {
    try {
        ...
        if (existingProjectUser) {
            return res.status(400).json({ message: "User is already working in this project." });
        }
        ...
        return res.status(500).json({ message: "Unable to add user to project.", error: error.message });
    }
};
...
const getProjectsUser = async (req, res) => {
    try {
        ...
        return res.status(500).json({ message: "Unable to fetch project users.", error: error.message });
    }
};
